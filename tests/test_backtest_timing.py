...
def test_missing_execution_price_fails_closed() -> None:
    market = pd.DataFrame(
        [
            {"date": "2026-01-02", "symbol": "AAA", "open": 10.0, "close": 10.0},
            {"date": "2026-01-05", "symbol": "BBB", "open": 20.0, "close": 21.0},
        ]
    )
    market["date"] = pd.to_datetime(market["date"])
    targets = pd.DataFrame(
    ...
    with pytest.raises(ValueError, match="missing execution prices for target symbols"):
...
def test_missing_price_for_held_symbol_fails_closed() -> None:
    market = pd.DataFrame(
        [
            {"date": "2026-01-02", "symbol": "AAA", "open": 10.0, "close": 10.0},
            {"date": "2026-01-05", "symbol": "AAA", "open": 10.0, "close": 10.0},
            {"date": "2026-01-06", "symbol": "BBB", "open": 20.0, "close": 21.0},
        ]
    )
    market["date"] = pd.to_datetime(market["date"])
    targets = pd.DataFrame(
    ...
    with pytest.raises(ValueError, match="missing market prices for held symbols"):
        run_next_open_backtest(
            market,
            targets,
            initial_cash=1_000.0,
